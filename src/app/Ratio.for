...
!***********************************************************************
!* Takes the Ratio of Two UDS Y-Series Files
!*
!* Language: Fortran
!*
...
!***********************************************************************
     ...
     & ONE_PASS, .TRUE., .TRUE. )
      PROMPT_1 = 'First (num) UDS file name/template/hitlist?'
      PROMPT_1(74:) = '[done]'
      PROMPT_2 = 'Second (div) UDS file name/template/hitlist?'
      CALL UDS_CONTROL_INIT( C )
      C.CHANFORM = 'Y' ! Only accept Y-series file input

      WRITE( *, '(//27X,A//)' ) '**  Ratio of UDS Files  **'
  ...
  110   CALL FILE_PROMPT( TEMPLATE_STR, PROMPT_2, DIR_OUT,
     &     ONE_PASS, HIT, NEW_HIT, FILE_NAME, EOF )
        ...
        ELSE IF ( FILE_NAME .EQ. ' ' ) THEN ! No more files
          IF ( ONE_PASS ) THEN
            CALL MSG_WRITE(
     &         '*** Operation skipped - No second (div) file',
     &         REPORT )
            GO TO 190
          ELSE
            GO TO 110
     ...
     &       '*** WARNING - Inconsistent X-units', REPORT )
          ...
          DO I = S.NFP, S.NLP
            IF ( U.Y(I) .EQ. 0. ) THEN
              IF ( S.Y(I) .EQ. 0. ) THEN ! 0/0: Set ratio=1
                S.Y(I) = 1.
              ELSE ! X/0: Set ratio=0
                S.Y(I) = 0.
              END IF
            ELSE
              S.Y(I) = S.Y(I) / U.Y(I)
