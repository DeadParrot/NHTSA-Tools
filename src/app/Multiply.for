...
!***********************************************************************
!* Multiplies UDS Y-Series Files Together
!*
!* Language: Fortran
!*
...
!***********************************************************************
     ...
     & XUNITS_1*20, YUNITS_1*20
      ...
      CALL MULTIPLY_CL( CL_ARG, NUM_ARGS, PROG_NAME, NPER_CL )
     ...
     & ONE_PASS, .TRUE., .TRUE. )
      ...
      WRITE( *, '(//29X,A//)' ) '**  Multiply UDS Files  **'
  ...
  199 CALL CLEAN_UP( HIT, REPORT, OUTPUT_LIST, DIR_OUT )
      ...
      SUBROUTINE MULTIPLY_CL( CL_ARG, NUM_ARGS, PROG_NAME, NPER_CL )
...
!***********************************************************************
      ...
      INTEGER NPER_CL ! Number of files per product [all]
     ...
     &       ( ARG(NS:) .EQ. 'HELP' ) ) THEN ! Show command line help
            ...
            WRITE( *, '(10X,A)' )
     &         '<num_per> :  Number of files per product [all]'
          END IF
          STOP ' '
     ...
     &     ( ARG(NS:NS+4) .EQ. 'NPER#' ) ) THEN
          ...
          IF ( ( IOS .NE. 0 ) .OR. ( NPER_CL .LE. 0 ) ) THEN
            WRITE( *, '(/2A)', IOSTAT=IOS )
     &         ' *** Illegal number of files per product: ',
     &         ARG(NS+5:L_TRIM(ARG))
            NPER_CL = 0
          END IF
          CL_ARG(I) = ' '
