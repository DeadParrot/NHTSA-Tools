...
!***********************************************************************
!* Computes the Max Difference of Two UDS Y-Series Files
!*
!* Language: Fortran
!*
...
!***********************************************************************
     ...
     & DONE
      ...
      DOUBLE PRECISION DIFF_MAX, DIFF_I
     ...
     & ONE_PASS, .TRUE., .TRUE. )
      ...
      WRITE( *, '(//24X,A//)' ) '**  Max Difference of UDS Files  **'
  ...
  110   CALL FILE_PROMPT( TEMPLATE_STR, PROMPT_2, DIR_OUT,
     ...
     &       '*** WARNING - Inconsistent X-units', REPORT )
      ...
      DIFF_MAX = 0.D0
      DO I = S.NFP, S.NLP
        DIFF_I = ABS( S.Y(I) - U.Y(I) )
        IF ( DIFF_I .GT. DIFF_MAX ) DIFF_MAX = DIFF_I
      END DO
