...
!***********************************************************************
!* Computes the maximum 15 msec Head Injury Criterion value
...
!***********************************************************************
     ...
     & PROMPT*79
      ...
      CALL HIC15_CL( CL_ARG, NUM_ARGS, PROG_NAME, UPDATE )
     ...
     & ONE_PASS, .TRUE., .TRUE. )
      ...
      WRITE( *, '(//14X,A//)' )
     & '**  15 Msec Head Injury Criterion Computation  **'
  ...
  100 CALL FILE_PROMPT( TEMPLATE_STR, PROMPT, DIR_OUT,
     ...
     &     '*** ERROR - Non-positive file time step value', REPORT )
      ...
      ! Compute the HIC15 value
      U.HICDTUP = 15. ! Set max HIC time window to 15 msec
  ...
  199 CALL CLEAN_UP( HIT, REPORT, OUTPUT_LIST, DIR_OUT )
      ...
      SUBROUTINE HIC15_CL( CL_ARG, NUM_ARGS, PROG_NAME, UPDATE )
