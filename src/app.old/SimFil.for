...
!***********************************************************************
     ...
     & ONE_PASS, .TRUE., .TRUE. )
      ...
      PI = 4.D0 * DATAN( 1.D0 )
      ALPHA = DLOG( PI / DACOS( 2. * ( 10.D0**(-.15D0) ) - 1.D0 ) ) /
     & DLOG( 4.D0 )
      BA(0) = 0.
      BAW(0) = 0.
      BVW(0) = 0.
      ...
      WRITE( *, '(//14X,A//)' )
     & '**  SimFil v.2 - Simultaneous Filtering Program  **'
  ...
  101   CALL GROUP_SETUP( ATYP, VTYP, DTYP, AFL, VFL, DFL,
     ...
     &     'limited to SimFil bound of: '//SHOW_INT, REPORT )
      ...
      END IF
      IF ( U.NLP .LT. 3 ) THEN
        CALL MSG_WRITE( '*** ERROR - Upper array index < 3 ',
     &     REPORT )
        GO TO 190
      END IF
     ...
     &     DFTTYP, OF_CTRL, AFIL, VFIL, DFIL, BATCH, GROUP, EOF )
      ...
      IF ( ( PTYP .EQ. 'F' ) .OR. ( PTYP .EQ. 'D' ) ) THEN
     ...
     &     V0V, A0V, ASV, B1V, B2V, D0D, V0D, A0D, ASD, B1D, B2D )
      ...
      IF ( ( PTYP .EQ. 'F' ) .OR. ( PTYP .EQ. 'D' ) ) THEN
        ...
        DO I = 1, NFREQ
          IF ( PTYP .EQ. 'F' ) THEN
            IF ( FREQ_C(I) .EQ. ' ' ) THEN
              IF ( FCUT_DEF_C(1:1) .EQ. 'N' ) THEN
                FCORI = FCUT_DEF
              ELSE
                FCORI = FCUT_DEF * .5D0
              END IF
              FSTPI = FSTP_DEF
            ELSE IF ( FREQ_C(I) .EQ. 'S' ) THEN
              FCORI = FSTOP
              FSTPI = FSTOP
            ELSE IF ( FREQ_C(I) .EQ. 'M' ) THEN
              FCORI = FMD
              FSTPI = FMD
            ELSE
              FCORI = FREQ(I) * .5D0
              FSTPI = FREQ(I) * 2.5D0
            END IF
            IF ( FCORI .GE. FMD ) THEN ! Check for subsampling
              IF ( ( DTF_C .EQ. ' ' ) .OR.
     &           ( DTF_C(1:1) .EQ. 'I' ) ) THEN
                FSTPI = 0.D0
              ELSE
                NSUB = NINT( DTF / DTT )
                IF ( EQUAL_DP( DTF, NSUB*DTT, 1.D-15 ) )
     &             FSTPI = 0.D0
              END IF
            END IF
          ELSE ! PTYP = 'D'
     ...
     &       KS_MAX, BA, NLP_S )
      ...
      IF ( PTYP .EQ. 'F' ) THEN ! Filter and output
        DO I = 1, NFREQ
     ...
     &       DIR_OUT, NO_INCR, REPORT, OUTPUT_LIST, BATCH, NFREQ )
        END DO
      ELSE IF ( PTYP .EQ. 'D' ) THEN ! Output DFT's
        DO I = 1, NFREQ
     ...
     &       DIR_OUT, NO_INCR, REPORT, OUTPUT_LIST, BATCH, NFREQ )
        END DO
      ELSE IF ( PTYP .EQ. 'R' ) THEN ! Raw output
...
!***********************************************************************
  ...
  101 V0_C = ' '
      IF ( INI_VAL ) THEN
        WRITE( *, '(/A,31X,A/'' >> '',$)' )
     &     ' Time-zero VELOCITY, IMPULSE, ... value?', '[default]'
        READ( *, '(A)', END=199 ) V0_C
        IF ( V0_C .NE. ' ' ) THEN
  ...
  102 D0_C = ' '
      IF ( INI_VAL ) THEN
        WRITE( *, '(/A,31X,A/'' >> '',$)' )
     &     ' Time-zero DISPLACEMENT, ... value?     ', '[default]'
        READ( *, '(A)', END=101 ) D0_C
        IF ( D0_C .NE. ' ' ) THEN
...
!***********************************************************************
     ...
     & 'Prefilter: ', U.PREF, ' Hz', 'Cutoff: ', U.FCUT, ' Hz'
      ...
      V0_D = 0.D0
      D0_D = 0.D0
      IF ( ( U.DIMSYS .EQ. 'ENG' ) .AND.
     & ( ( U.YUNITS .EQ. 'G''S' ) .OR.
     & ( U.YUNITS .EQ. 'MILES/HOUR' ) .OR.
     & ( U.YUNITS .EQ. 'INCHES' ) .OR.
     & ( U.YUNITS .EQ. 'FEET' ) ) ) THEN
        ATYP = 'ACCELERATION'
        VTYP = 'VELOCITY'
        DTYP = 'DISPLACEMENT'
        IF ( ( U.YTYP .EQ. 'REL. VELOCITY' ) .OR.
     &     ( U.YTYP .EQ. 'DEFLECTION' ) ) THEN
          VTYP = 'REL. VELOCITY'
          DTYP = 'DEFLECTION'
        ELSE IF ( U.YTYP .EQ. 'LENGTH' ) THEN
          DTYP = 'LENGTH'
        ELSE IF ( U.YTYP .EQ. 'POSITION' ) THEN
          DTYP = 'POSITION'
        END IF
        AFL = 'A'
        VFL = 'V'
      ...
      ELSE IF ( ( U.DIMSYS .EQ. 'MET' ) .AND.
     & ( ( U.YUNITS .EQ. 'G''S' ) .OR.
     & ( U.YUNITS .EQ. 'KILOMETERS/HOUR' ) .OR.
     & ( U.YUNITS .EQ. 'MILLIMETERS' ) .OR.
     & ( U.YUNITS .EQ. 'METERS' ) .OR.
     & ( U.YUNITS .EQ. 'CENTIMETERS' ) ) ) THEN
        ATYP = 'ACCELERATION'
        VTYP = 'VELOCITY'
        DTYP = 'DISPLACEMENT'
        IF ( ( U.YTYP .EQ. 'REL. VELOCITY' ) .OR.
     &     ( U.YTYP .EQ. 'DEFLECTION' ) ) THEN
        ...
        IF ( U.YUNITS .EQ. 'METERS' ) THEN
          DUN = U.YUNITS
          VDFAC = 1.D3 / 3600.D0
          ADFAC = 9.80665D0
        ELSE IF ( U.YUNITS .EQ. 'CENTIMETERS' ) THEN
          DUN = U.YUNITS
          VDFAC = 1.D5 / 3600.D0
          ADFAC = 9.80665D2
        ELSE
        ...
        END IF
        V0_D = DP_TRAN( U.INIVEL )
      ELSE IF ( ( U.DIMSYS .EQ. 'SI' ) .AND.
     & ( ( U.YUNITS .EQ. 'METERS/SEC**2' ) .OR.
     & ( U.YUNITS .EQ. 'METERS/SEC' ) .OR.
     & ( U.YUNITS .EQ. 'METERS' ) .OR.
     & ( U.YUNITS .EQ. 'MILLIMETERS' ) .OR.
     & ( U.YUNITS .EQ. 'CENTIMETERS' ) ) ) THEN
        ATYP = 'ACCELERATION'
     ...
     &     ( U.YTYP .EQ. 'DEFLECTION' ) ) THEN
        ...
        VUN = 'METERS/SEC'
        AVFAC = 1.D0
        IF ( U.YUNITS .EQ. 'MILLIMETERS' ) THEN
          DUN = U.YUNITS
          VDFAC = 1.D3
          ADFAC = 1.D3
        ELSE IF ( U.YUNITS .EQ. 'CENTIMETERS' ) THEN
        ...
        END IF
        V0_D = DP_TRAN( U.INIVEL )
      ELSE IF ( ( U.YUNITS .EQ. 'DEGREES/SEC**2' ) .OR.
     & ( U.YUNITS .EQ. 'DEGREES/SEC' ) .OR.
     & ( U.YUNITS .EQ. 'DEGREES' ) ) THEN
        ...
        VUN = 'DEGREES/SEC'
        DUN = 'DEGREES'
      ELSE IF ( ( U.YUNITS .EQ. 'RADIANS/SEC**2' ) .OR.
     & ( U.YUNITS .EQ. 'RADIANS/SEC' ) .OR.
     & ( U.YUNITS .EQ. 'RADIANS' ) ) THEN
        ATYP = 'ANGULAR ACCELERATION'
        VTYP = 'ANGULAR VELOCITY'
        DTYP = 'ANGULAR DISPLACEMENT'
        AFL = 'A'
        ...
        VUN = 'RADIANS/SEC'
        DUN = 'RADIANS'
      ELSE IF ( ( U.YUNITS .EQ. 'POUNDS' ) .OR.
     & ( U.YUNITS .EQ. 'POUND-SECONDS' ) ) THEN
        ATYP = 'FORCE'
        VTYP = 'IMPULSE'
        DTYP = 'IMPULSE x TIME'
        IF ( U.YTYP .EQ. 'STATIC FORCE' ) ATYP = U.YTYP
        AFL = 'F'
        VFL = 'I'
        DFL = '2'
        AUN = 'POUNDS'
        VUN = 'POUND-SECONDS'
        DUN = 'POUND-SEC**2'
      ELSE IF ( ( U.YUNITS .EQ. 'NEWTONS' ) .OR.
     & ( U.YUNITS .EQ. 'NEWTON-SECONDS' ) ) THEN
        ATYP = 'FORCE'
        VTYP = 'IMPULSE'
        DTYP = 'IMPULSE x TIME'
        IF ( U.YTYP .EQ. 'STATIC FORCE' ) ATYP = U.YTYP
        ...
        VUN = 'NEWTON-SECONDS'
        DUN = 'NEWTON-SEC**2'
      ELSE IF ( ( U.YUNITS .EQ. 'FOOT-POUNDS' ) .OR.
     & ( U.YUNITS .EQ. 'INCH-POUNDS' ) ) THEN
        ATYP = 'ENERGY'
        VTYP = 'ENERGY x TIME'
        ...
        VFL = '1'
        DFL = '2'
        IF ( U.YUNITS .EQ. 'INCH-POUNDS' ) THEN
          AUN = U.YUNITS
        ...
        END IF
      ELSE IF ( ( U.YUNITS .EQ. 'JOULES' ) .OR.
     & ( U.YUNITS .EQ. 'METER-NEWTONS' ) ) THEN
        ATYP = 'ENERGY'
        VTYP = 'ENERGY x TIME'
        DTYP = 'ENERGY x TIME**2'
        AFL = 'E'
        ...
        VUN = 'JOULE-SECONDS'
        DUN = 'JOULE-SEC**2'
      ELSE IF ( ( U.YUNITS .EQ. 'POUND-FEET' ) .OR.
     & ( U.YUNITS .EQ. 'POUND-FOOT-SECONDS' ) ) THEN
        ATYP = 'TORQUE'
      ...
      ELSE IF ( ( U.YUNITS .EQ. 'POUND-INCHES' ) .OR.
     & ( U.YUNITS .EQ. 'POUND-INCH-SECONDS' ) ) THEN
        ATYP = 'TORQUE'
        VTYP = 'ANGULAR IMPULSE'
        DTYP = 'ANG. IMPULSE x TIME'
        AFL = 'T'
      ...
      ELSE IF ( ( U.YUNITS .EQ. 'NEWTON-METERS' ) .OR.
     & ( U.YUNITS .EQ. 'NEWTON-METER-SECONDS' ) ) THEN
        ATYP = 'TORQUE'
        VTYP = 'ANGULAR IMPULSE'
        DTYP = 'ANG. IMPULSE x TIME'
        AFL = 'T'
      ...
      ELSE
        CALL MSG_WRITE( '*** Unsupported units: '//
     &     U.YUNITS//' => Generic integrals', REPORT )
        ATYP = U.YTYP
  ...
  104   V0_C = ' '
     ...
     &       VTYP(:L_TRIM(VTYP)), ' value = ', V0D
      ...
      FSTOP = 0.D0
      IF ( U.PREF .GT. 0. ) FSTOP = 2.5D0 * U.PREF
      IF ( FSTOP .EQ. 0.D0 ) THEN
        FSTOP = MAX( SF, 2.5D0*FC )
      ELSE IF ( FC .GT. 0.D0 ) THEN
...
!***********************************************************************
  ...
  103 IF ( PTYP .EQ. 'F' ) THEN ! Set cutoff frequency
        CALL FREQ_SET( 'Cutoff ', FCUT_DEF_C, NFREQ, FREQ_C, FREQ,
     &     EOF )
      ELSE IF ( PTYP .EQ. 'D' ) THEN ! Set stop frequency
        CALL FREQ_SET( 'DFT max', FSTP_DEF_C, NFREQ, FREQ_C, FREQ,
     &     EOF )
      END IF
      IF ( EOF ) GO TO 101

      ! Set up filtering or DFT spec's
  104 IF ( PTYP .EQ. 'F' ) THEN ! Filter
        CALL FILTER_SET( ATYP, VTYP, DTYP, DTT, FTD,
     &     FTF, FTF_C, DTF, DTF_C, NFREQ, OF_CTRL,
     &     AFIL, VFIL, DFIL, BATCH, GROUP, EOF )
      ELSE IF ( PTYP .EQ. 'D' ) THEN ! DFT
        CALL DFT_SET( ATYP, VTYP, DTYP, DFTTYP, NFREQ, OF_CTRL,
     &     AFIL, VFIL, DFIL, BATCH, EOF )
      ELSE IF ( PTYP .EQ. 'R' ) THEN ! Raw
...
!***********************************************************************
  ...
  101 IF ( GROUP ) THEN
     ...
     &     ' DFT final time (sec)?', '[',FTT,']'
      ...
      IF ( FTD_C .EQ. ' ' ) THEN ! Use tail
        FTD = FTT
      ELSE ! Assign value
        READ( FTD_C, '(BN,F25.0)', IOSTAT=IOS ) FTD
        IF ( ( IOS .NE. 0 ) .OR. ( FTD .LT. 0.D0 ) ) THEN
...
!***********************************************************************
  ...
  101 WRITE( *, '(/3A,11X,A/29X,5A/31X,5A/33X,5A/'' >> '',$)' )
     ...
     & '( ',DFL,' - ',DTYP(:L_TRIM(DTYP)),' )'
      ...
      VSM = .FALSE.
      DSM = .FALSE.
      IF ( SMDOM .EQ. ' ' ) THEN ! Default A-domain smoothing
        ASM = .TRUE.
      ELSE IF ( SMDOM .EQ. 'N' ) THEN ! No smoothing
        RETURN
      ELSE ! Check assign smoothing domain flags
        DO I = 1, LEN_TRIM( SMDOM )
          ...
          IF ( C .EQ. AFL ) THEN
            ASM = .TRUE.
          ELSE IF ( C .EQ. VFL ) THEN
            VSM = .TRUE.
          ELSE IF ( C .EQ. DFL ) THEN
            DSM = .TRUE.
          ELSE IF ( ( C .NE. ' ' ) .AND. ( C .NE. ',' ) .AND.
     &       ( C .NE. '/' ) .AND.  ( C .NE. TAB ) ) THEN
  ...
  102 WRITE( *, '(/2A,15X,A/'' >> '',$)' ) ' Time-zero tail',
     ...
     & '['//FCUT_DEF_C//']'
      ...
      IF ( FS0_C .EQ. ' ' ) THEN
        ! Use default
      ELSE IF ( FS0_C .NE. 'N' ) THEN ! Use specified frequency
        READ( FS0_C, '(BN,F25.0)', IOSTAT=IOS ) FS0
        IF ( ( IOS .NE. 0 ) .OR. ( FS0 .LE. 0.D0 ) ) THEN
  ...
  103 WRITE( *, '(/2A,21X,A/'' >> '',$)' ) ' End tail',
     ...
     & '['//FCUT_DEF_C//']'
      ...
      IF ( FSN_C .EQ. ' ' ) THEN
        ! Use default
      ELSE IF ( FSN_C .NE. 'N' ) THEN ! Use specified frequency
        READ( FSN_C, '(BN,F25.0)', IOSTAT=IOS ) FSN
        IF ( ( IOS .NE. 0 ) .OR. ( FSN .LE. 0.D0 ) ) THEN
          WRITE( *, '(/A)' ) ' *** Invalid frequency value'
          GO TO 103
        END IF
      END IF
  ...
  199 EOF = .TRUE.
      ...
      SUBROUTINE FREQ_SET( FREQ_NAME, FREQ_DEF_C, NFREQ, FREQ_C, FREQ,
     & EOF )
...
!***********************************************************************
     ...
     & FRS_C*25, DEF_ACT*7, FREQ_DEF_O*15
      ...
      IF ( IOS .EQ. 0 ) THEN
        FREQ_DEF_O = ' = '//FREQ_DEF_C(:L_TRIM(FREQ_DEF_C))//
     &     ' (Hz)'
      ELSE
        FREQ_DEF_O = ' '
      END IF
      WRITE( *, '(/3A)' ) ' Enter up to 100 ', FREQ_NAME,
     & ' frequencies:'
  101 WRITE( *, '(/1X,2A,I3,A,41X,A/9X,3A/9X,A/9X,A/'' >> '',$)' )
     ...
     & '( M - Max frequency )'
      ...
      FRS_C = LJUST( FRS_C )
      FRS = 0.D0
      IF ( FRS_C .EQ. ' ' ) THEN ! Default frequency
        IF ( NFREQ .EQ. 0 ) THEN
          IF ( .NOT. DEF_FR ) THEN
            DEF_FR = .TRUE.
          ELSE
        ...
        ELSE
          RETURN
        END IF
      ELSE IF ( FRS_C .EQ. 'D' ) THEN ! Default frequency
        IF ( .NOT. DEF_FR ) THEN
          DEF_FR = .TRUE.
        ELSE
          WRITE( *, '(/A)' ) ' *** Repeat frequency ignored'
          GO TO 101
        END IF
      ELSE IF ( FRS_C .EQ. 'S' ) THEN ! Stop frequency
        IF ( .NOT. STOP_FR ) THEN
          STOP_FR = .TRUE.
        ELSE
          WRITE( *, '(/A)' ) ' *** Repeat frequency ignored'
          GO TO 101
        END IF
      ELSE IF ( FRS_C .EQ. 'M' ) THEN ! Max frequency
        IF ( .NOT. MAX_FR ) THEN
          MAX_FR = .TRUE.
        ELSE
          WRITE( *, '(/A)' ) ' *** Repeat frequency ignored'
      ...
      ELSE ! Specified frequency
        ...
        IF ( ( IOS .NE.0 ) .OR. ( FRS .LT. 0.D0 ) ) THEN
          WRITE( *, '(/A)' ) ' *** Invalid frequency'
          GO TO 101
        END IF
        DO I = 1, NFREQ-1
          IF ( FRS .EQ. FREQ(I) ) THEN
            WRITE( *, '(/A)' ) ' *** Repeat frequency ignored'
            GO TO 101
          END IF
        END DO
...
!***********************************************************************
  ...
  101 IF ( GROUP ) THEN
     ...
     &     ' Final time (sec)?', '[',FTD,']'
      ...
      IF ( FTF_C .EQ. ' ' ) THEN ! Use default
        FTF = FTD
      ELSE ! Specified final time
        READ( FTF_C, '(BN,F25.0)', IOSTAT=IOS ) FTF
        IF ( ( IOS .NE. 0 ) .OR. ( FTF .LT. 0.D0 ) ) THEN
  ...
  102 IF ( GROUP ) THEN
     ...
     & THEN ! Use default
      ...
      IF ( ( NFREQ .GT. 1 ) .AND. ( .NOT. BATCH ) ) THEN
  103   WRITE( *, '(/A,25X,A/'' >> '',$)' )
     ...
     &     '[group]'
      ...
      IF ( ( BATCH ) .OR. ( OF_CTRL .EQ. 'G' ) ) THEN
  104   WRITE( *, 601 ) ATYP
        READ( *, '(A)', END=101 ) AFIL
        IF ( ( AFIL .EQ. ' ' ) .OR.
     &     ( UPCASE( LJUST( AFIL ) ) .EQ. 'Y' ) ) THEN
          AFIL = ' '
        ELSE IF ( UPCASE( LJUST( AFIL ) ) .EQ. 'N' ) THEN
          AFIL = 'N'
        ELSE
          WRITE( *, '(/A)' ) ' *** Invalid response'
          GO TO 104
  ...
  105   WRITE( *, 601 ) VTYP
        READ( *, '(A)', END=101 ) VFIL
        IF ( ( VFIL .EQ. ' ' ) .OR.
     &     ( UPCASE( LJUST( VFIL ) ) .EQ. 'Y' ) ) THEN
          VFIL = ' '
        ELSE IF ( UPCASE( LJUST( VFIL ) ) .EQ. 'N' ) THEN
          VFIL = 'N'
        ELSE
          WRITE( *, '(/A)' ) ' *** Invalid response'
          GO TO 105
  ...
  106   WRITE( *, 601 ) DTYP
        READ( *, '(A)', END=101 ) DFIL
        IF ( ( DFIL .EQ. ' ' ) .OR.
     &     ( UPCASE( LJUST( DFIL ) ) .EQ. 'Y' ) ) THEN
          DFIL = ' '
        ELSE IF ( UPCASE( LJUST( DFIL ) ) .EQ. 'N' ) THEN
          DFIL = 'N'
        ELSE
          WRITE( *, '(/A)' ) ' *** Invalid response'
          GO TO 106
...
!***********************************************************************
     ...
     & DTAV2, DTV0, DTAD6, AI, ASUM, A6, A6SUM
      ...
      DO I = -1, NFP, -1
        AI = A(I)
        V(I) = V0 - DTAV2 * ( AI + ASUM )
        D(I) = D0 + I * DTV0 +
     &     DTAD6 * ( AI + A6SUM - ( 3 * I + 1 ) * A0 )
        ASUM = 2 * AI + ASUM
        A6 = 6 * AI + A6
        A6SUM = A6 + A6SUM
      END DO
      ...
      ELSE
        ...
        DO I = -1, NFP+1, -1
          ...
          IF ( MOD( I, 2 ) .EQ. 0 ) THEN
            D(I) = D0 - DTVD3 * ( VI + VESUM )
            VESUM = VESUM + 4 * VM + 2 * VI
          ELSE
            D(I) = D0 - DTVD3 * ( VI + VOSUM ) + DTA0
            VOSUM = VOSUM + 4 * VM + 2 * VI
          END IF
        END DO
        I = NFP
        VI = VM
        A(I) = ( -7 * VI + 6 * V(I+1) + 3 * V(I+2) - 2 * V(I+3) ) *
     &   DTVA / 3
        IF ( MOD( I, 2 ) .EQ. 0 ) THEN
          D(I) = D0 - DTVD3 * ( VI + VESUM )
        ELSE
...
!***********************************************************************
     ...
     & VM2, VM, VI, VP, VP2, VP3
      ...
      DP2 = D(I+2)
      DP3 = D(I+3)
      V(I) = ( -7 * DI + 6 * DP + 3 * DP2 - 2 * DP3 ) * DTDV / 3
      DM2 = D(NLP-3)
      DO I = NFP+1, NLP-1
        DM = DI
        DI = DP
        ...
        V(I) = ( DP - DM ) * DTDV
      END DO
      V(NLP) = ( 7 * DP - 6 * DI - 3 * DM + 2 * DM2 ) * DTDV / 3
      ...
      VP2 = V(I+2)
      VP3 = V(I+3)
      A(I) = ( -7 * VI + 6 * VP + 3 * VP2 - 2 * VP3 ) * DTVA / 3
      VM2 = V(NLP-3)
      DO I = NFP+1, NLP-1
        VM = VI
        VI = VP
        ...
        A(I) = ( VP - VM ) * DTVA
      END DO
      A(NLP) = ( 7 * VP - 6 * VI - 3 * VM + 2 * VM2 ) * DTVA / 3
...
!***********************************************************************
     ...
     & TWON, PIT, PIT2, BAVFAC, BADFAC, W0, FMD, FMC, AN, VN, DN
      ...
      IF ( FTD_C .EQ. ' ' ) THEN ! Use signal tail
        NDD = NLP_I
        FTD = FTT
      ELSE ! Assign specified value
        NDD = MAX( MIN( INT( (FTD/DTT+1.D0)*(1.D0-1.D-14) ), NLP_I),
     &     2 )
        FTD = NDD * DTT
      END IF
...
!***********************************************************************
     ...
     &     REPORT )
      ...
      IF ( FS0_C .EQ. ' ' ) THEN
        ...
        FS0_C = FCUT_DEF_C
      END IF
      IF ( FS0_C .NE. 'N' ) THEN ! Use specified frequency
        ...
        H(0) = 2 * OMS
        DO I = 1, NS
          H(I) = ( 1.D0 + DCOS( OMSRHO * I ) ) * DSIN( OMS * I ) / I
        END DO
        H_SET = .TRUE.

        IF ( NS .GT. 0 ) THEN ! Perform smoothing
     ...
     &       D0D, V0D, A0D, ASD, B1D, B2D )
          ...
          DO I = 1, NPT-1
            ...
            PITI = PIT * I
            PIT2I = PIT2 * I
            ST = D0D + V0D * T + A0D * TS + ASD * TC +
     &         B1D * DSIN( PITI ) + B2D * DSIN( PIT2I )
            SPT = V0V + A0V * T + ASV * TS + B1V * DCOS( PITI ) +
     &         B2V * DCOS( PIT2I )
            SPPT = A0A + ASA * T + B1A * DSIN( PITI ) +
     &         B2A * DSIN( PIT2I )
            VI = V(I)
            DI = D(I)
            A(I) =
     &         ( VI * VAFAC2 + T * A(I) - SPT * VAFAC2 + TPT * SPPT )
     &         / TP
            V(I) =
     &         ( DI / VDFAC + T * VI - ST / VDFAC + TPT * SPT ) / TP
            D(I) = ( T * DI + TPT * ST ) / TP
          END DO
  ...
  101 END IF
      ...
      IF ( FSN_C .EQ. ' ' ) THEN
        ...
        FSN_C = FCUT_DEF_C
      END IF
      IF ( FSN_C .NE. 'N' ) THEN ! Use specified frequency

        ! Set up smoothing parameters
        IF ( ( FSN .NE. FS0 ) .OR. ( .NOT. H_SET ) ) THEN
          ...
          DO I = 1, NS
            H(I) =
     &         ( 1.D0 + DCOS( OMSRHO * I ) ) * DSIN( OMS * I ) / I
          END DO
        END IF
     ...
     &         B2A * DSIN( PIT2I )
            ...
            DI = D(NI)
            A(NI) =
     &         ( -VI * VAFAC2 + T * A(NI) - SPT * VAFAC2 + TPT * SPPT )
     &         / TP
            V(NI) =
     &         -( DI / VDFAC - T * VI - ST / VDFAC + TPT * SPT ) / TP
            D(NI) = ( T * DI + TPT * ST ) / TP
...
!***********************************************************************
  ...
  199 WRITE( *, '(/A)' )
     & ' *** WARNING - Specified tail smoothing is not computable'
      FAIL = .TRUE.
      RETURN
...
!***********************************************************************
     ...
     & CUTF, CORF, STPF
      ...
      IF ( CUTF_C .EQ. ' ' ) THEN ! Default frequency
        IF ( FCUT_DEF_C(1:1) .EQ. 'N' ) THEN ! Max frequency
          FILMOD = 'M'
          CUTF = FMD
          CORF = FMD
          STPF = FMD
        ...
        ELSE ! Default frequency
          ...
          U.FCUT = FCUT_DEF
          U.FCOR = FCUT_DEF * .5D0
          U.FSTP = FCUT_DEF * 2.5D0
        END IF
      ELSE IF ( CUTF_C .EQ. 'S' ) THEN ! Stop frequency mode
        FILMOD = 'S'
        CUTF = FSTOP
        CORF = FSTOP
        STPF = FSTOP
        ...
        U.FCOR = CF
        U.FSTP = SF
      ELSE IF ( CUTF_C .EQ. 'M' ) THEN ! Max frequency mode
        FILMOD = 'M'
        CUTF = FMD
        CORF = FMD
        STPF = FMD
...
!***********************************************************************
      ...
      IF ( STPF_C .EQ. ' ' ) THEN ! Default frequency
        IF ( FSTP_DEF_C(1:1) .EQ. 'M' ) THEN ! Max frequency
          STPF = FMD
        ELSE ! Default frequency
          STPF = FSTP_DEF
        END IF
      ELSE IF ( STPF_C .EQ. 'S' ) THEN ! Stop frequency
        STPF = FSTOP
      ELSE IF ( STPF_C .EQ. 'M' ) THEN ! Max frequency
        STPF = FMD
...
!***********************************************************************
     ...
     & AI, ANI
      ...
      DO I = 1, NDDHM
        NI = NDD - I
        AI = A(I) - A0A - ASA * ( I * DTT ) - B1A * DSIN( PIT * I ) -
     &     B2A * DSIN( PIT2 * I )
        ANI = A(NI) - A0A - ASA * ( NI * DTT ) -
     &     B1A * DSIN( PIT * NI ) - B2A * DSIN( PIT2 * NI )
        AO(I) = AI + ANI
        AE(I) = AI - ANI
      END DO
      IF ( NO .GT. NDDHM ) THEN
        AO(NO) = A(NO) - A0A - ASA * ( NO * DTT ) -
     &     B1A * DSIN( PIT * NO ) - B2A * DSIN( PIT2 * NO )
      END IF
...
!***********************************************************************
     ...
     & V0V, A0V, ASV, B1V, B2V, T, TN, VI, VNI
      ...
      DO I = 1, NDDHM
        ...
        NI = NDD - I
        TN = NI * DTT
        VI = V(I) - V0V - A0V * T - ASV * T**2 -
     &     B1V * DCOS( PIT * I ) - B2V * DCOS( PIT2 * I )
        VNI = V(NI) - V0V - A0V * TN - ASV * TN**2 -
     &     B1V * DCOS( PIT * NI ) - B2V * DCOS( PIT2 * NI )
        VO(I) = VI - VNI
        VE(I) = VI + VNI
      END DO
      IF ( NE .GT. NDDHM ) THEN
        VE(NE) = V(NE) - V0V - A0V * ( NE * DTT ) -
     &     ASV * ( NE * DTT )**2 -
     &     B1V * DCOS( PIT * NE ) - B2V * DCOS( PIT2 * NE )
      END IF
...
!***********************************************************************
     ...
     & D0D, V0D, A0D, ASD, B1D, B2D, T, TN, DI, DNI
      ...
      DO I = 1, NDDHM
        ...
        NI = NDD - I
        TN = NI * DTT
        DI = D(I) - D0D - V0D * T - A0D * T**2 - ASD * T**3 -
     &     B1D * DSIN( PIT * I ) - B2D * DSIN( PIT2 * I )
        DNI = D(NI) - D0D - V0D * TN - A0D * TN**2 - ASD * TN**3 -
     &     B1D * DSIN( PIT * NI ) - B2D * DSIN( PIT2 * NI )
        DOD(I) = DI + DNI
        DE(I) = DI - DNI
      END DO
      IF ( NO .GT. NDDHM ) THEN
        DOD(NO) = D(NO) - D0D - V0D * ( NO * DTT ) -
...
!***********************************************************************
     ...
     & DEF_FILE*(*), OUT_FILE*(*), DIR_OUT*(*), FILCOD
      ...
      IF ( NFREQ .GT. 1 ) WRITE( *, '(//1X,F12.3,A)', IOSTAT=IOS_W )
     & CUTF, ' (Hz) output files:'

      ! Set final time value
      IF ( FTF_C .EQ. ' ' ) THEN
        FTF = FTD
      ELSE IF ( FTF .GT. FTD ) THEN
     ...
     &     FTD, ' (sec)'
      ...
      ISUB = 0
      IF ( ( DTF_C .EQ. ' ' ) .OR. ( DTF_C .EQ. 'I' ) ) THEN
        ! Input time step
        ISUB = 1
        DTF = DTT
      ELSE IF ( DTF_C(1:1) .EQ. 'I' ) THEN ! Subsampling requested
        READ( DTF_C(2:), '(BN,I24)', IOSTAT=IOS ) ISUB
        DTF = DTT * ISUB
      ELSE IF ( CORF .GE. FMD ) THEN ! Check for subsampling
        NSUB = NINT( DTF / DTT )
      ...
      NLP_F = NINT( FTF / DTF )
      DO WHILE ( NLP_F * DTF .GT. FTD )
        NLP_F = NLP_F - 1
      END DO
      IF ( NLP_F .GT. NLP_S ) THEN
     ...
     &     NLP_S
      ...
      IF ( ( BATCH ) .OR. ( OF_CTRL .EQ. 'G' ) ) THEN ! Default names
        IF ( AFIL .NE. 'N' ) THEN
          AFIL = DEF_FILE
  ...
  103   OUT_FILE = DEF_FILE
     ...
     &     ( DFIL .EQ. 'N' ) ) THEN
      ...
      IF ( ( CORF .GE. FMD ) .AND. ( ISUB .GT. 0 ) ) THEN ! Subsample
     ...
     &     A0A, ASA, V0V, A0V, ASV, D0D, V0D, A0D, ASD, AF, VF, DF,
     &     NLP_S )
      ...
      U.NFP = 0
      U.NLP = NLP_F
      IF ( ISUB .EQ. 1 ) THEN ! Use input (single precision) DEL
        U.DEL = DEL_I
      ELSE
        U.DEL = DTF
      END IF
      IF ( ( FILMOD .EQ. 'S' ) .OR. ( FILMOD .EQ. 'M' ) ) THEN
        U.STATUS = 'FILTERED'
      ELSE IF ( ( ( FC .GT. 0.D0 ) .AND.
     & ( STPF .GT. FC * (.5+1.D-15) ) ) .OR.
     & ( ( CF .GT. 0.D0 ) .AND. ( STPF .GT. CF * (1.+1.D-15) ) ) .OR.
     & ( ( U.PREF .GT. 0. ) .AND.
     & ( STPF .GT. U.PREF * (.5+1.D-15) ) ) )
     & THEN
        WRITE( *, '(/A)' )
     &     ' *** WARNING - Filter overlap distortion present'
        U.STATUS = 'DISTORTED'
      ELSE IF ( STPF .GT. FMC * (1.+1.D-14 ) ) THEN
        WRITE( *, '(/A)' )
     &     ' *** WARNING - Filter window truncated by Nyquist limit'
        U.STATUS = 'NYQUIST LIMITED'
      ELSE IF ( DTF * STPF .GE. .5D0 ) THEN
        WRITE( *, '(/A)' )
     &     ' *** WARNING - Aliasing present - Time step too large'
        U.STATUS = 'ALIASED'
      ELSE
        U.STATUS = 'FILTERED'
      END IF
      ...
      IF ( AFIL .NE. 'N' ) THEN
        ...
        DO I = U.NFP, U.NLP
          U.Y(I) = AF(I)
        END DO
        CALL UDS_PUT( AFIL, U, C, REPORT, OUTPUT_LIST,
     &     DIR_OUT, .TRUE., IOS )
      END IF
      IF ( VFIL .NE. 'N' ) THEN
        U.YTYP = VTYP
        ...
        DO I = U.NFP, U.NLP
          U.Y(I) = VF(I)
        END DO
        CALL UDS_PUT( VFIL, U, C, REPORT, OUTPUT_LIST,
     &     DIR_OUT, .TRUE., IOS )
      END IF
      IF ( DFIL .NE. 'N' ) THEN
        U.YTYP = DTYP
        ...
        DO I = U.NFP, U.NLP
          U.Y(I) = DF(I)
        END DO
        CALL UDS_PUT( DFIL, U, C, REPORT, OUTPUT_LIST,
     &     DIR_OUT, .TRUE., IOS )
      END IF
...
!***********************************************************************
     ...
     & OLAMN, OLAMD, OLAM, ELAMN, ELAMD, ELAM, BAWJ
      ...
      DO J = 1, KC
        H(J) = 1.
      END DO
      IF ( ( STPF .GT. CORF ) .AND. ( KS .GT. KC ) ) THEN
        FDIF = STPF - CORF
        CFR = CORF / FDIF
        W0FR = W0 / FDIF
        DO J = KC+1, KS
          H(J) =
     &       ( 1.D0 + DCOS( ( ( J * W0FR - CFR )**ALPHA ) * PI ) ) / 2
        END DO
      END IF
      ...
      ELSE
        ...
        DO J = 1, KS, 2
          ...
          BAW(J) = BAWJ
          IF ( VFIL .NE. 'N' ) BVW(J) = BAWJ * BAVFAC / J
          IF ( DFIL .NE. 'N' ) BDW(J) = BAWJ * BADFAC / ( J**2 )
        END DO
      END IF
...
!***********************************************************************
     ...
     & AO, AE, VO, VE, DD, DE, AI, VI, DI, TFAC, SJ
      ...
      NDF = NINT( FTD / DTF )
      NDFMAX = 2 * NLP_S
      IF ( DTF .GT. FTD ) RMETH = 'S'
      DO WHILE ( RMETH .EQ. ' ' )
        IF ( NDF .GT. NDFMAX ) THEN ! Use slow method
          RMETH = 'S'
        ELSE IF
     &     ( DMOD( DTF * (1.+1.D-15), FTD/NDF ) .LT. DTF * (1.D-12 ) )
     &     THEN
          RMETH = 'F'
          DTF = NSPF * FTD / NDF
      ...
      IF ( RMETH .EQ. 'F' ) THEN ! Fast method values
        NDFH = NDF / 2
        NDF2 = NDF * 2
        PIT = PI / NDF
        IF ( ( ( AFIL .NE. 'N' ) .OR. ( DFIL .NE. 'N' ) ) .AND.
     &     ( NDF .NE. NDFSIN ) ) THEN
          ...
          NDFSIN = NDF
        END IF
        IF ( ( VFIL .NE. 'N' ) .AND. ( NDF .NE. NDFCOS ) ) THEN
          ...
          NDFCOS = NDF
        END IF
      ELSE IF ( RMETH .EQ. 'S' ) THEN ! Slow method values
        NDF = -1
        WRITE( *, '(/A/4X,A)' )
     ...
     &     ' DFT time span and filter time step lack large gcd'
      ...
      IF ( ( RMETH .EQ. 'F' ) .AND. ( NSPF .EQ. 1 ) .AND.
     & ( NLP_F .GT. NDFH*1.2 ) ) THEN
        ! Use fast method with time folding
        IF ( ( AFIL .NE. 'N' ) .AND. ( VFIL .NE. 'N' ) .AND.
     &     ( DFIL .NE. 'N' ) ) THEN
          ...
          DO I = 1, NDFH
            ...
            DF(I) = DD + DE
            NDFI = NDF - I
            IF ( NDFI .LE. NLP_F ) THEN
              AF(NDFI) = AO - AE
              VF(NDFI) = VE - VO
              DF(NDFI) = DD - DE
            END IF
          END DO
        ELSE IF ( ( AFIL .NE. 'N' ) .AND. ( VFIL .NE. 'N' ) ) THEN
          ! Reconstruct A and V
          DO I = 1, NDFH
            AO = 0.D0
            AE = 0.D0
            ...
            END IF
          END DO
        ELSE IF ( ( AFIL .NE. 'N' ) .AND. ( DFIL .NE. 'N' ) ) THEN
          ! Reconstruct A and D
          DO I = 1, NDFH
            AO = 0.D0
            AE = 0.D0
            ...
            END IF
          END DO
        ELSE IF ( ( VFIL .NE. 'N' ) .AND. ( DFIL .NE. 'N' ) ) THEN
          ! Reconstruct V and D
          DO I = 1, NDFH
            VO = 0.D0
            VE = 0.D0
        ...
        ELSE IF ( AFIL .NE. 'N' ) THEN
          ...
          DO I = 1, NDFH
            ...
            END DO
            AF(I) = AO + AE
            IF ( NDF-I .LE. NLP_F ) AF(NDF-I) = AO - AE
          END DO
        ELSE IF ( DFIL .NE. 'N' ) THEN
          ! Reconstruct D
          DO I = 1, NDFH
            DD = 0.D0
            DE = 0.D0
            ...
            END DO
            DF(I) = DD + DE
            IF ( NDF-I .LE. NLP_F ) DF(NDF-I) = DD - DE
          END DO
        ELSE IF ( VFIL .NE. 'N' ) THEN
          ! Reconstruct V
          DO I = 1, NDFH
            VO = 0.D0
            VE = 0.D0
            ...
            END DO
            VF(I) = VE + VO
            IF ( NDF-I .LE. NLP_F ) VF(NDF-I) = VE - VO
          END DO
        END IF
      ELSE IF ( RMETH .EQ. 'F' ) THEN ! Use fast method w/o time folding
        IF ( ( AFIL .NE. 'N' ) .AND. ( VFIL .NE. 'N' ) .AND.
     &     ( DFIL .NE. 'N' ) ) THEN
          ! Reconstruct A, V, and D
          IL = 0
...
!***********************************************************************
     ...
     & A0A, ASA, V0V, A0V, ASV, D0D, V0D, A0D, ASD, T, TS, TC
      ...
      DO I = 1, NLP_F
        ...
        TC = T**3
        AF(I) = AF(I) + A0A + ASA * T
        VF(I) = VF(I) + V0V + A0V * T + ASV * TS
        DF(I) = DF(I) + D0D + V0D * T + A0D * TS + ASD * TC
      END DO

      ! Set DFT end tails
      IF ( NLP_F .EQ. NDF ) THEN ! Filter end tail = DFT end tail
        AF(NLP_F) = AN
        VF(NLP_F) = VN
        DF(NLP_F) = DN
      END IF
...
!***********************************************************************
  ...
  103   OUT_FILE = DEF_FILE
     ...
     & KS, KSF, BA, VFIL, DFIL, DFTTYP, BAW, BVW, BDW, NLP_S )
      ...
      U.NLP = KSF
      U.DEL = W0
      IF ( ( ( FC .GT. 0.D0 ) .AND. ( STPF .GT. FC*(.5+1.D-15) ) )
     & .OR. ( ( CF .GT. 0.D0 ) .AND. ( STPF .GT. CF*(1.+1.-15) ) )
     & .OR. ( ( U.PREF .GT. 0. ) .AND.
     & ( STPF .GT. U.PREF*(.5+1.D-15) ) ) ) THEN
        WRITE( *, '(/A)' )
     &     ' *** WARNING - DFT overlap distortion present'
        U.STATUS = 'DISTORTED'
      ELSE
        U.STATUS = 'COMPACTED'
      END IF
...
!***********************************************************************
      ...
      DOUBLE PRECISION BAVFAC, BADFAC, B1A, B2A, B1V, B2V, B1D, B2D,
     & BAWJ
...
!***********************************************************************
     ...
     &       'signal final time of ', FTT, ' (sec)'
      ...
      NFP_O = NINT( STF / DTF )
      DO WHILE ( NFP_O * DTF .LT. STT )
        NFP_O = NFP_O + 1
      END DO
      IF ( NFP_O .LT. NFP_S ) THEN
        WRITE( *, '(/A,I6)', IOSTAT=IOS_W )
     ...
     &     NFP_S
      ...
      NLP_O = NINT( FTF / DTF )
      DO WHILE ( NLP_O * DTF .GT. FTT )
        NLP_O = NLP_O - 1
      END DO
      IF ( NLP_O .GT. NLP_S ) THEN
        WRITE( *, '(/A,I6)', IOSTAT=IOS_W )
     &     ' *** WARNING - Final index set to max value of: ',
     &     NLP_S
        NLP_O = NLP_S
     ...
     &     ( DFIL .EQ. 'N' ) ) THEN
      ...
      ELSE ! Interpolate
        ...
        DO I = NFP_O, NLP_O
          TI = I * DTF
          II = NINT( TI / DTT )
          IF ( ( II * DTT .GT. TI ) .OR. ( II .GE. NLP_I ) )
     &       II = II - 1
          IP = II + 1
          TII = II * DTT
          FRAC = ( TI - TII ) / DTT
          AF(I) = A(II) + FRAC * ( A(IP) - A(II) )
          VF(I) = V(II) + FRAC * ( V(IP) - V(II) )
          DF(I) = D(II) + FRAC * ( D(IP) - D(II) )
        END DO
